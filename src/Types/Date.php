...
class Date extends \SimplePHP\Core\SimpleObject {
     ...
     public function format($pattern = null) {
          ...
          if (!$this->getError()) {
               return str_replace(['y', 'm', 'd', 'h', 'i', 's', 'Y', 'H'], [substr($this->year, 2), str_pad($this->month, 2, '0', STR_PAD_LEFT), str_pad($this->day, 2, '0', STR_PAD_LEFT), ($this->hour > 12 ? str_pad($this->hour - 12, 2, '0', STR_PAD_LEFT) : str_pad($this->hour, 2, '0', STR_PAD_LEFT)), str_pad($this->minute, 2, '0', STR_PAD_LEFT), str_pad($this->second, 2, '0', STR_PAD_LEFT), $this->year, str_pad($this->hour, 2, '0', STR_PAD_LEFT)], $pattern);
          } else {
               return '';
          }
     }
